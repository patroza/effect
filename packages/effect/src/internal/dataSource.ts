...
export const makeBatched = <A extends Request.Request<any, any>, R>(
  run: (requests: RA.NonEmptyReadonlyArray<A>) => Effect.Effect<void, never, R>
): RequestResolver.RequestResolver<A, R> =>
    ...
    (requests) => {
      if (requests.length > 1) {
        return core.forEachSequentialDiscard(requests, (block) => {
          ...
          if (!RA.isNonEmptyReadonlyArray(filtered)) {
            return core.void
          }
...
export const fromFunction = <A extends Request.Request<any>>(
  ...
  makeBatched((requests: RA.NonEmptyReadonlyArray<A>) =>
...
export const fromFunctionBatched = <A extends Request.Request<any>>(
  f: (chunk: RA.NonEmptyReadonlyArray<A>) => Iterable<Request.Request.Success<A>>
): RequestResolver.RequestResolver<A> =>
  makeBatched((as: RA.NonEmptyReadonlyArray<A>) =>
    Effect.forEach(
      f(as),
...
> =>
  makeBatched<A, any>((requests: RA.NonEmptyReadonlyArray<A>) => {
