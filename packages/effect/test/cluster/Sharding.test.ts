  ...
  it.effect("interrupts entities and reacquires shards after lock storage recovers", () =>
    Effect.gen(function*() {
      ...
      yield* Effect.gen(function*() {
        ...
        assert.isTrue(warnings.some((message) =>
          globalThis.Array.isArray(message) && message.includes("Shard lock storage still unhealthy, retrying")
        ))
      }).pipe(Effect.provide(layer), Effect.withLogger(logger), Effect.scoped)
    }))

  it.effect("reacquires shards when the empty liveness probe succeeds while lock refresh is hung", () =>
    Effect.gen(function*() {
      const storageState = makeFailoverStorageState()
      const runnerStorage = Layer.effect(
        RunnerStorage.RunnerStorage,
...
interface FailoverStorageState {
  blackholed: boolean
  /** Hang only non-empty lock refresh, so the empty liveness probe can succeed. */
